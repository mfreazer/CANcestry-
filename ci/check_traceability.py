...
``Test strategy.md`` section 10 requires traceability completeness to be checked
in CI ("Every High-priority requirement shall have at least one test", "Every
test shall reference at least one requirement ID", "Traceability completeness
shall be checked in CI"). This script is that check. It is deliberately
dependency-free and read-only: it never edits the record, it reports what does
not hold.

Rules enforced
--------------
1. ``docs/trace/traceability.csv`` has the documented five-column header and no
   empty or duplicated row.
2. ``verification_method`` and ``status`` use the documented vocabulary.
3. Requirement ids are shaped like ``SW-FR-*``/``SYS-*``/``QA-*``.
4. Every ``SW-FR-*``/``SYS-*`` id used in the CSV is actually defined in
   ``docs/software/SwRS.md`` or ``docs/SyRS.md`` (no invented or misspelled ids).
...
6. Every ``passing`` row resolves: its ``test_id`` appears literally in a source
   artifact under ``tests/``, ``examples/``, ``tools/`` or ``ci/``. A passing
   status therefore always points at something a reviewer can open.
7. Every test source (``*.c`` under ``tests/``) cites at least one requirement id
   or QA review item.
8. Every requirement whose rows are all ``planned``/``failed`` is named in the
   deferred ledger, so the "not verified yet" set is explicit rather than
   implied. ``QA-*`` review items are exempt (they are tasks, not requirements).

The deferred ledger is the section of ``docs/trace/traceability.md`` whose
heading contains "deferred ledger". It may name ids literally (``SYS-NF-004``) or
as an inclusive range (``SW-FR-PKG-001..007``); both forms are expanded here.
Only the table's first column is normative, so the rationale column may mention
requirements that are verified elsewhere without deferring them.

Usage:
    python3 ci/check_traceability.py <repo-root>

Exit codes:
    0  every rule holds
    1  at least one rule failed (each failure is printed with its rule number)
"""
...
CSV_HEADER = ["requirement_id", "artifact_id", "verification_method", "test_id", "status"]
METHODS = ("test", "inspection", "demonstration")
STATUSES = ("passing", "planned", "failed")

# Where a passing row's test id must appear: executable artifacts, not prose.
...
class Report(object):
    """Collects failures grouped by rule."""
...
def load_rows(root, report):
    """Rule 1: structure. Returns the parsed rows (possibly empty)."""
    ...
    for number, row in enumerate(rows[1:], start=2):
        if len(row) != len(CSV_HEADER):
            report.fail(1, "line %d has %d fields, expected %d" % (number, len(row),
                                                                   len(CSV_HEADER)))
            continue
        ...
        key = (record["requirement_id"], record["artifact_id"], record["test_id"])
        if key in seen:
            report.fail(1, "duplicate row for %s / %s / %s" % key)
        seen.add(key)
        records.append(record)
    return records, [row for row in rows[1:] if len(row) == len(CSV_HEADER)]


def check_vocabulary(records, report):
    """Rule 2: controlled values."""
    for record in records:
        ...
        if record["status"] not in STATUSES:
            report.fail(2, "%s: status %r is not one of %s" %
                        (record["requirement_id"], record["status"], ", ".join(STATUSES)))
        if not REQUIREMENT_ID.match(record["requirement_id"]):
            report.fail(3, "%s is not a recognised requirement id" % record["requirement_id"])


def read_defined_ids(root, report):
    """Every requirement id the SwRS and SyRS define."""
    defined = set()
...
def read_deferred_ids(root, report):
    """Requirement ids named in the deferred ledger of the traceability record."""
    ...
    # Only the first column of the ledger table is normative: the rationale
    # column may legitimately mention requirements that are already verified
    # (for example "SYS-FR-014 determinism is claimed by the gateway harness"),
    # and such a mention must not be read as deferring them.
    deferred = set()
...
def collect_artifacts(root):
    """Concatenated text of the source artifacts a test id may resolve to."""
    chunks = []
...
def check_rows(records, defined, deferred, artifacts, report):
    ...
    for record in records:
        ...
        if (record["status"] == "passing" and
                record["test_id"] not in artifacts):
...
def check_coverage(defined, csv_ids, deferred, report):
    """Rule 5: no requirement is silently absent."""
...
def check_tests_cite_requirements(root, defined, report):
    """Rule 7: every test source cites a requirement or QA review item."""
...
def main(argv):
    ...
    report = Report()
    records, _ = load_rows(root, report)
    ...
    check_coverage(defined, {record["requirement_id"] for record in records}, deferred,
                   report)
    checked_tests = check_tests_cite_requirements(root, defined, report)
    ...
    if report.failures:
        ...
        for failure in report.failures:
            print("      %s" % failure)
        return 1
    print("PASS: the traceability record is complete and every passing row resolves")
    return 0
