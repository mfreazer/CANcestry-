...
Version: 0.1.1 — H-04 review correction. Qualification is **pending**.
...
`provisional: true`, `credibility_level: CL0`; its per-pulse records are pending.
The hardware ledger's HW-FR-004 virtual-bench row is `sim-pending` with no
closure evidence/hash. The numerical tests still run and must pass.
...
## 3. Enforced evidence disposition and future plot-data
...
requires every pulse's coverage and status to permit a pass. The traceability
...
ledger; all must fail.

**No plot-data schema, plot-data file or render exists on this branch.** The
visual-evidence implementation is still open as #36. H-04 does not silently
implement or weaken that separate contract. A future plot must propagate the
source/per-pulse `pending` status; a successful `regression_pass` run log is
not authority to emit a `passing` plot. The numerical run log explicitly
carries pending qualification status as well.
