#!/usr/bin/env python3
"""Fixture stub renderer for the H-03 gate tests (deterministic, no clock).

Implements the renderer CLI contract documented in ci/check_hw_evidence.py and
docs/hw/visual-evidence-plan.md:

    <renderer_tool> --plot-data <path> --version <version> --output -

The artifact is derived only from the plot-data file (which pins the evidence
hash), so re-rendering the same plot-data is byte-identical: that is exactly
what ``--rerender --strict`` checks. The stub draws plain polylines, a hatched
tolerance band, the embedded cancestry-provenance comment and the visible
footer, and it uses the declared non-color encodings. It is a test fixture,
not evidence.
"""
